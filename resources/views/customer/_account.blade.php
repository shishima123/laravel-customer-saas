<div class="kt-portlet no-box-shadow">
    ...
    <!--begin::Form-->
    <form class="kt-form" action="{{ route('customers.account', $customer) }}" method="POST">
        @csrf
        <div class="kt-portlet__body pt-0 pb-3">
            ...
            <div class="kt-portlet__head-label">
                <h3 class="kt-section__title title fw-500 mb-3">
                    {{ __('message.text.password.change_pass') }}
                </h3>
            </div>
            @if(auth()->user()->isUser())
            <div class="form-group row">
                <div class="col-xl-4 col-lg-6">
                    <label for="current_password" class="form-label">{{ __('message.auth.current_password') }}</label>
                    <input id="current_password" type="password" name="current_password" class="form-control @error('current_password') is-invalid @enderror" placeholder="{{ __('message.user.no_information') }}" autocomplete="off">
